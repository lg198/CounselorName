...
public class DatabaseManager {
	...
	public static void init(String driver, String uri) throws DatabaseNotFoundException, DatabaseConnectionException, DatabaseInitException {
		try {
			Class.forName(driver);
		} catch (ClassNotFoundException e) {
			throw new DatabaseNotFoundException("The driver '" + driver + "' was not found on the classpath!", e);
		}
	...
	public static String[] parseScript(String s) {
		try {
			...
			BufferedReader br = new BufferedReader(new InputStreamReader(DatabaseManager.class.getResourceAsStream(script)));
	...
	public static List<Student> searchStudents(String query) throws SQLException {
		...
		ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM student WHERE UPPER(firstname || \" \" || lastname) LIKE UPPER('%" 
				+ query + "%')");
		List<Student> students = new ArrayList<Student>();
		while (rs.next()) {
			Student s = new Student(rs.getString("id"), rs.getString("firstname"), rs.getString("lastname"));
			students.add(s);
		}
		return students;
	}
	...
	public static Student addStudent(String fn, String ln) throws SQLException {
		...
		connection.createStatement().execute(String.format("INSERT INTO student (id, firstname, lastname) values (\"%s\", \"%s\", \"%s\")",
				id, fn, ln));
